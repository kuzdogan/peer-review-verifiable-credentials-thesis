    ...
    <div className='shadow-lg rounded-xl p-8 bg-white relative overflow-hidden m-8'>
      <div className='w-full'>
        <p className='text-gray-800 text-xl font-medium mb-2'>
          <Link to={`/manuscripts/${id}`}>{title}</Link>
        </p>
        <p className='text-gray-800 text-md font-medium mb-2'>Author: {`${author.firstName} ${author.lastName}`}</p>
        ...
        {abstract && (
          <div>
            ...
            <p className='text-gray-400 text-sm mb-4 whitespace-pre-wrap'>{abstract}</p>
          </div>
        )}
      </div>
      ...
      {content && (
        <div>
          ...
          <p className='text-gray-700 text-sm mb-4 whitespace-pre-wrap'>{content}</p>
        </div>
      )}
      {children}
